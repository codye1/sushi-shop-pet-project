        ...
        <div>
            <div className="home-slider">
                ...
                <Swiper
                    ...
                    >
                    {slide.map((s)=>
                        <SwiperSlide className="home-swiper-slide" key={s.key}><img src={s.img} alt="" /></SwiperSlide>
                    )}
                </Swiper>
                </div>
            </div>
            Доставка суші
            {error?<div>Помилка</div>:isLoading?<div>Загрузка...</div>:data?<ProductList product={data}/>: <div>Ничего нет</div> }
            <Banner/>
            <AboutSushiMaster/>
        </div>
    );
