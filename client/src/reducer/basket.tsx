...
const basketSlice = createSlice({
  ...
  reducers: {
    ...
    deleteById(state, action: PayloadAction<IProduct>) {
      ...
      if (state.basket[action.payload.id]?.length <= 0)
    ...
    deleteAllElementsByIdFromBasket(state, action: PayloadAction<IProduct>) {
      for (const addition of action.payload.additions) {
        if (state.additionsIds[addition])
          state.additionsIds[addition] -=
            state.basket[action.payload.id].length;
        if (state.additionsIds[addition] <= 0)
          delete state.additionsIds[addition];
      }
      delete state.basket[action.payload.id];
    },
...
export const {
  ...
  deleteById,
  setBasketPageStatus,
  deleteAllElementsByIdFromBasket,
  deleteAllElementsFromBasket,
  calculateAdditions,
} = basketSlice.actions;
