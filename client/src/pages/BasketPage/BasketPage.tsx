...
const BasketPage = () => {
  ...
  const { data: additions } = useGetProductsByIdsQuery(
    Object.keys(additionsIds).length > 0 ? Object.keys(additionsIds) : ['null']
  );
  ...
  return (
    <>
      ...
      {products.length > 0 && (
        <BasketReccomendationSlider products={products} />
      )}
    </>
  );
};
