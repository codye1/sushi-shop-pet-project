...
const ImageSection = () => {
  ...
  return (
    <div className="photo-section d-flex align-center">
      <img
        className="show-img"
        src={imgUser && imgUser.length > 0 ? imgUser : picture}
        alt=""
      />
      <button
        className="pointer"
        onClick={() => {
          setModalUploadImgVissible(true);
        }}
      >
        {imgUser && imgUser.length > 0 ? 'Редагувати ' : 'Додати '}фото
      </button>
      <CustomModal
        ...
        <div className="modal-file-upload">
          ...
          {img ? (
            <>
              ...
              <TwoButtons
                applyTitle={t(
                  'account.personal.sections.image.two-buttons-crop.applyTitle'
                )}
                cancelTitle={t(
                  'account.personal.sections.image.two-buttons-crop.cancelTitle'
                )}
                onApply={() => {
                  croppAndSaveImg();
                  onCloseModal();
