...
export const useFilter = () => {
  const filterByLabels = (labels: string[], products: IProduct[]) => {
    if (labels.length > 0) {
  ...
  const sortPrice = (price: string, products: IProduct[]) => {
  ...
  const sortWeight = (weight: string, products: IProduct[]) => {
    const productsTemp: IProduct[] = JSON.parse(JSON.stringify(products));
    if (weight != '4') {
      productsTemp.sort((a, b) => {
        if (weight == '5') return a.harch.weight > b.harch.weight ? 1 : -1;
        else return a.harch.weight < b.harch.weight ? 1 : -1;
      });
    }
  ...
  const filtrate = (
    formFiltrAndSort: IFormFilterAndSort,
    products: IProduct[],
